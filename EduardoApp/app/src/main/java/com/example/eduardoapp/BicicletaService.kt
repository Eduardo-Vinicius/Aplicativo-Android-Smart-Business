...
//}
    ...
    fun getBicicletas (context: Context): List<Bicicleta> {
        ...
        if (AndroidUtils.isInternetDisponivel(context)){
            val url = "$host/bicicletas"
            val json = HttpHelper.get(url)

            Log.d(TAG, json)
            var f = parserJson<List<Bicicleta>>(json)
            ...
            val dao = DatabaseManager.getBicicletaDAO()
            return dao.getAll()
        }
    }
    fun saveOffline (f: Bicicleta) {
        val dao = DatabaseManager.getBicicletaDAO()
        if (! existeB(f)) {
            dao.insert(f)
        }
    ...
    fun existeB(f: Bicicleta): Boolean {
        val dao = DatabaseManager.getBicicletaDAO()
        return dao.getById(f.id) != null
    }


    fun save(f: Bicicleta) {
        val url = "$host/bicicletas"
...
//        return parserJson(json)
    ...
    fun delete(f: Bicicleta): Response {
        val url = "$host/bicicletas/${f.id}"
